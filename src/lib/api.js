...
// New/unseen reservations — powers the badge + "new reservations" modal.
export function getNewReservations(pin) {
  return fetch('/api/new-reservations', { headers: { 'x-staff-pin': pin } }).then(json);
}
...
// --- Push notifications -----------------------------------------------------
export function savePushSubscription(subscription, pin) {
  return fetch('/api/push-subscribe', {
    method: 'POST', headers: pinHeaders(pin), body: JSON.stringify(subscription)
  }).then(json);
}

export function removePushSubscription(endpoint, pin) {
  return fetch('/api/push-subscribe', {
    method: 'DELETE', headers: pinHeaders(pin), body: JSON.stringify({ endpoint })
  }).then(json);
}
