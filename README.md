...
The staff manager shows a red count badge (iOS-style) on a bell icon whenever
there are reservations no one has looked at yet — across *any* Sunday, not
just the one currently open.

- Tapping the bell opens a modal listing every unseen reservation, newest
  first.
- Tapping an entry in that modal jumps straight to that reservation's Sunday,
  scrolls to it, and briefly highlights it (a gold glow that fades after ~2.5s)
  — and it's removed from the badge/modal, since opening it counts as having
  seen it.
- Tapping a **push notification** does the same thing directly: it deep-links
  to `/staff?sunday=...&highlight=...`, which jumps to that Sunday, highlights
  the exact reservation, and marks it seen — instead of just opening the app
  to wherever it happened to be.
- The badge also polls every 30 seconds while the page is open, so it stays
  current even without a push notification arriving.

There's no per-card "NEW" tag or filter toggle in the normal daily list by
design — the bell + modal is the only place "unseen" surfaces, to keep the
day-to-day list exactly as clean as before.
...
- `unseen:reservations` — a sorted set (score = created-at ms, member =
  `{sunday}::{id}`) tracking every reservation no staff member has opened yet.
  This is what powers the badge/modal above; entries are removed the moment a
  reservation is opened via the modal or its push notification (or deleted).
