...
serverless functions + Redis (Upstash/Vercel KV) + Resend email.
...
In the Vercel dashboard → **Storage** → **Create** → choose a Redis store
(Upstash) → **Connect** it to this project. Vercel injects the connection
credentials automatically — you do **not** set these by hand. The code reads
either naming scheme (`KV_REST_API_URL`/`KV_REST_API_TOKEN` or
...
| Redis vars       | Auto     | —                                           | Injected when you connect the store in step 3    |

`.env.example` mirrors this for local development (`vercel env pull` fetches
the Redis vars for `vercel dev`).
