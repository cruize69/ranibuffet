...
import { bookableSundays } from './_lib/sundays.js';
...
async function createReservation(req, res) {
  ...
  // cannot book a date that is no longer available.
  const allowed = bookableSundays();
  if (!allowed.includes(sunday))
    return res.status(400).json({
      error: 'That date is no longer available. Please refresh and choose again.',
      bookableSundays: allowed
    });
  ...
  const emailResult = await sendConfirmationEmail(reservation, prettyDate(sunday));
...
async function getReservations(req, res) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(sunday))
    return res.status(400).json({ error: 'A valid week is required.' });
