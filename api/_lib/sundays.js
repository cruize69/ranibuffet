...
// (today when it's Sunday, so the list stays visible during service) and
// forward. Never earlier than LAUNCH_SUNDAY, matching the customer picker.
export function staffSundays(now = new Date(), count = 6) {
  const p = etParts(now);
  const today = ymd(p.year, p.month, p.day);
  const daysUntilSunday = (7 - p.weekday) % 7;
  let thisWeekSunday = addDays(today, daysUntilSunday);
  if (thisWeekSunday < LAUNCH_SUNDAY) thisWeekSunday = LAUNCH_SUNDAY;
  return Array.from({ length: count }, (_, i) => addDays(thisWeekSunday, i * 7));
}

// Validate a date a customer jumped to via the "choose another date" calendar:
// must be a real Sunday, on/after the earliest bookable Sunday right now, and
// within a year of it (a sane upper bound — the quick 3-button picker is the
// common path, this is just for planning further ahead).
export function isValidBookableSunday(dateStr, now = new Date()) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateStr || '')) return false;
  const [y, m, d] = dateStr.split('-').map(Number);
  if (new Date(Date.UTC(y, m - 1, d)).getUTCDay() !== 0) return false;
  const min = earliestBookableSunday(now);
  if (dateStr < min) return false;
  const max = addDays(min, 365);
  return dateStr <= max;
}

export const STAFF_LAUNCH_SUNDAY = LAUNCH_SUNDAY;

// Validate a date a staff member jumped to via the calendar: must be a real
// Sunday, in YYYY-MM-DD form, and not earlier than the program's launch date.
export function isValidStaffSunday(dateStr) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateStr || '')) return false;
  if (dateStr < LAUNCH_SUNDAY) return false;
  const [y, m, d] = dateStr.split('-').map(Number);
